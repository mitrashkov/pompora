...
export default function AppShell() {
  ...
  const toggleStickyScroll = useCallback(() => {
    ...
    setIsStickyScrollEnabled((v) => {
      ...
      updateEditorOptions({ stickyScroll: { enabled: next } });
  ...
  const toggleRenderWhitespace = useCallback(() => {
    ...
    setIsRenderWhitespaceEnabled((v) => {
      ...
      updateEditorOptions({ renderWhitespace: next ? "all" : "none" });
  ...
  const toggleRenderControlChars = useCallback(() => {
  ...
  const [isStickyScrollEnabled, setIsStickyScrollEnabled] = useState(true);
  ...
  const applyUiZoom = useCallback((z: number) => {
    ...
    root.style.setProperty("--pompora-ui-zoom", String(clamped));
  ...
  const getUiZoom = useCallback((): number => {
  ...
  const kbRaw = useCallback(
    ...
    (id: string) => {
      ...
      return String(settings.keybindings?.[id] ?? DEFAULT_KEYBINDINGS[id] ?? "");
  ...
  const togglePrimarySidebar = useCallback(() => {
    ...
    setIsPrimarySidebarOpen((v) => {
      ...
      if (!next) {
        ...
        lastExplorerWidthRef.current = explorerWidth;
        ...
        const w = lastExplorerWidthRef.current;
